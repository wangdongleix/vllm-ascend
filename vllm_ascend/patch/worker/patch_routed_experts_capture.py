# Adapted from https://github.com/vllm-project/vllm/blob/main/vllm/model_executor/layers/fused_moe/routed_experts_capturer.py
...
def capture(
    ...
    # Graph warmup uses at most the configured decode capture size.  Validate
    # the first real (larger) prefill separately so a graph-warmup marker can
    # never be mistaken for proof that natural-rollout rows were captured.
    if (
        kimi_full_r3
        and token_num_per_dp > 16
        and not getattr(self, "_kimi_full_r3_real_prefill_logged", False)
    ):
        ids_cpu = selected[:, :actual_topk].detach().cpu()
        ordered = torch.sort(ids_cpu.to(torch.int64), dim=-1).values
        duplicate = ordered[:, 1:] == ordered[:, :-1]
        if bool(duplicate.any()):
            bad_row = int(torch.nonzero(duplicate, as_tuple=False)[0, 0].item())
            raise RuntimeError(
                "Kimi full R3 capture produced duplicate expert IDs before "
                "worker D2H: "
                f"layer={layer_id}, row={bad_row}, "
                f"routes={ids_cpu[bad_row].tolist()}"
            )
        marker = (
            "Kimi vLLM full R3 REAL PREFILL CAPTURE ACTIVE: "
            f"layer={layer_id} rows={token_num_per_dp} "
            f"first_ids={ids_cpu[0].tolist()} "
            f"local_payload_rows={int(topk_ids.shape[0])} "
            f"stored_payload_rows={int(selected.shape[0])} "
            "duplicates=False"
        )
        print(marker, flush=True)
        self._kimi_full_r3_real_prefill_logged = True
